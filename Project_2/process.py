import os
import pandas as pd

def count_duplicates():
    # اسم ملف الإكسيل المدخل
    input_file = 'A1.xlsx'
    # اسم ملف الإكسيل الجديد الذي سيحتوي على التكرارات
    output_file = 'A1_duplicates.xlsx'
    
    # التأكد من وجود الملف في نفس المجلد
    if not os.path.exists(input_file):
        print(f"خطأ: الملف '{input_file}' غير موجود في هذا المجلد!")
        return

    try:
        # قراءة ملف الإكسيل (يفترض وجود عمود واحد)
        df = pd.read_excel(input_file)
        
        if df.empty:
            print("الملف فارغ!")
            return
            
        # الحصول على اسم العمود الأول تلقائياً
        column_name = df.columns[0]
        
        # حساب تكرار كل قيمة وتحويلها إلى DataFrame جديد
        df_counts = df[column_name].value_counts().reset_index()
        
        # تسمية الأعمدة الجديدة
        df_counts.columns = [column_name, 'Count (التكرار)']
        
        # حفظ النتيجة في ملف إكسيل جديد
        df_counts.to_excel(output_file, index=False)
        print(f"تم بنجاح! تم إنشاء ملف التكرارات باسم: {output_file}")
        
    except Exception as e:
        print(f"حدث خطأ أثناء معالجة الملف: {e}")

if __name__ == '__main__':
    count_duplicates()
